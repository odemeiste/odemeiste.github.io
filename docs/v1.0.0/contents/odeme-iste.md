...
- Alacaklı ÖHS, ödeme iste talebi Borçlu ÖHS de oluşturulduğu anda durumunu “Yanıt Bekleniyor” olarak kaydeder.Sonrasında; Alacaklı ÖHS ÖdemeIsteTalebi için Borçlu ÖHS tarafından dönülen yanıt başlığında yer alan imza bilgisinin hatalı olması durumunda işlemi devam ettirmez ve OdemeIsteIptal servisini çağırarak iptal eder. İptal detay kodu olarak "14" - Alacaklı ÖHS Ödeme İste Hatalı İmza Nedeniyle İptal Etti  Nedeniyle İptal Etti dönülmelidir. 14 iptal detay kodu sadece POST/odeme-iste yanıtında hatalı imza olması durumunda kullanılacaktır. 
...
- Borçlu ÖHS tarafında SGZ süresi aşılan bir kayıt için ödeme iste kaydının iptal durumu gerçekleşmez ve Alacaklı ÖHS’ye OdemeIsteYanit nesnesi ile iletilmezse Alacaklı ÖHS tarafından SGZ+DTS süresinden sonra ilgili ödeme iste kaydının iptali gerçekleştirilebilir.
